...
void BaseLine::SFGD_BaseLine(TH1F* &hFEBCH_HG,TH1F* &hFEBCH_LG, std::pair<unsigned int,unsigned int> NFEBCh,std::vector<int> HG_LG){
    ...
    for(unsigned int i = 0; i < hFEBCH_full.size(); i++ ) {
        ...
        if (peak.GetHeight() > BASELINE_HEIGHT && peak.GetPosition() != 0)
            baseline[el].push_back({HG_LG[i], peak.GetPosition()});
    }
}
...
if(iCh.first._DAC == 2)
xml_data[el].push_back({int((left_value->_par_1+right_value->_par_2)/2),2.0});
if(iCh.first._DAC == 3)
xml_data[el].push_back({int((left_value->_par_1+right_value->_par_2)/2),3.0});
}
}
return xml_data;
}
