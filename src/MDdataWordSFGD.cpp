...
ostream & operator<<(ostream &s, MDdataWordSFGD &dw) {
    ...
    switch (dt) {
        case MDdataWordSFGD:: GateHeader:
            s << "Gate Header  BoardId: " << dw.GetBoardId();
        ...
        case MDdataWordSFGD::GateTime:
            s << "Gate TimeID Spill time: " << dw.GetGateTime();
            break;
        ...
        case MDdataWordSFGD::EventDone:
            s << "Event Done, Feb BoardId: " << dw.GetBoardId();
            s << " Gate Type:" << dw.GetGateType();
            s << " Gate Number:" << dw.GetGateNumber();
            break;

        case MDdataWordSFGD::SpecialWord:
            s << "SpecialWord: ";
            if (dw.GetSpecialWord()==0x10000) {
                s << "BoardId: " << dw.GetBoardId() << " Readout End";
            } else if (dw.GetSpecialWord()==0x20000) {
                s << "BoardId: " << dw.GetBoardId() << " GTS TAG Reset";
            } else if (dw.GetSpecialWord()==0x40000) {
                s << "BoardId: " << dw.GetBoardId() << " Gate Number Reset";
            } else if (dw.GetSpecialWord()==0x60000) {
                s << "BoardId: " << dw.GetBoardId() << " GTS TAG + GATE NUMBER RESET";
            } else {
                s << "BoardId: " << dw.GetBoardId();
                if (dw.GetSpecialWord() == 0x100 ) {
                    s << " L0_T FIFO Full";
                } else if (dw.GetSpecialWord() == 0x80 ) {
                    s << " L1_T FIFO Full";
                } else if (dw.GetSpecialWord() == 0x40 ) {
                    s << " L2_T FIFO Full";
                } else if (dw.GetSpecialWord() == 0x20 ) {
                    s << " L1_A FIFO Full";
                } else if (dw.GetSpecialWord() == 0x10 ) {
                    s << " L2_A FIFO Full";
                } else if (dw.GetSpecialWord() == 0x8 ) {
                    s << " L3 FIFO Full";
                } else if (dw.GetSpecialWord() == 0x4 ) {
                    s << " GTS Lost";
                } else if (dw.GetSpecialWord() == 0x2 ) {
                    s << " SYNC Decoder Error";
                } else if (dw.GetSpecialWord() == 0x1 ) {
                    s << " EXT CLK Error";
                }
            }
