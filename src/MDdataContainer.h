...
class MDdataContainer {
 ...
 public:
  MDdataContainer(void *d=0, unsigned long long int s=1)
  ...
  unsigned char* GetDataPtr(unsigned long long int i=0) {
    if ( i<_size ) return &_data[i];
    else return NULL;
  }

  uint32_t* Get32bWordPtr(unsigned long long int i=0) {
  ...
  virtual void Dump(){
    ...
    unsigned long long int iword;
    unsigned long long int nword=_size>>2;
