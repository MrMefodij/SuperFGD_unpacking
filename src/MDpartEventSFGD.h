...
struct gateTrailer{
    ...
    unsigned int gateType_;
    unsigned int gateNumber_;
...
class MDpartEventSFGD : public MDdataContainer {
 ...
 public:
  ...
  void AddTagIdShift(MDdataWordSFGD &dw, unsigned int shift);
  ...
  unsigned int GetHitShift(unsigned int ih, unsigned int ich);
  ...
  unsigned int GetTagIdShift(unsigned int ih, unsigned int ich)     { return GetHitShift(ih, ich); };
  ...
  gateTrailer GetGateTrailer()                                      {return _gateTrailer;}
...
private:
    ...
    std::vector<unsigned int>  _tadIdShift[SFGD_FEB_NCHANNELS];
