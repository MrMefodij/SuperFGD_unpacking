...
void ThresholdStudy::FindThreshold(std::vector<TH1F*>& hFEBCH, std::vector<unsigned int> DAC, unsigned int bin_entries){
    ...
    for (auto j = 0; j < hFEBCH.size(); j++) {
        auto bin = hFEBCH[j]->FindFirstBinAbove();

        while (hFEBCH[j]->GetBinContent(bin) < bin_entries && hFEBCH[j]->GetEntries() > 0 && bin > 0) {
            hFEBCH[j]->SetBinContent(bin, 0);
            bin = hFEBCH[j]->FindFirstBinAbove();
            try{
                if(bin < 0)
                    throw DAC[j] ;
            }
            ...
            {
                std::cerr << "Error: Something wrong with DAC " << exception << '\n';
            }
        }
...
TGraph* ThresholdStudy::PrintThreshold(double tan_angle){
    ...
    for(int i = 0; i < _threshold.size(); i++) {
        ...
        if(i != 0 && abs(double(_threshold[i]._ADC) - double(_threshold[i-1]._ADC)) / (_threshold[i]._DAC - _threshold[i-1]._DAC) > tan_angle
        ) {
            continue;
        }
    ...
    if(_section.size() > 1) {
        for (auto i = 0; i < _section.size() - 1; i++) {
            ...
            double DAC_value = (-par[1] + sqrt(par[1] * par[1] - 4 * par[2] * (par[0] - (_section[i]._y + THRESHOLD_VALUE * (_pars[i+1]-_pars[i]))))) / 2 / par[2];
            std::cout << i + THRESHOLD_VALUE <<": "<<  round(DAC_value) << std::endl;
